  ...
  $.fn.shijian = function(opt) {
    function cPlugin(o, emlnum) {
      ...
      sjObj.defaults = {
        ...
        df_box : function() {
          return $("</div><div class='df-box "
            + (sjObj.opt.alwaysShow ? "alwaysShow" : "") + " "
            + sjObj.opt.boxClassName + "' style='line-height:"
            + sjObj.opt.height + "px;'></div>")
        },
        df_main : function() {
        ...
        df_btn : function() {
          ...
          return $("<div class='df-btn' style='height:"
            + sjObj.opt.height + "px'><div class='df-ok'>"
            + sjObj.opt.okText + "</div><div class='df-no'>"
            + sjObj.opt.cancelText + "</div></div>")
        },
        df_wrap : function() {
        ...
        getArr : function() {
          ...
          for (var i = 0; i < 61; i++) {
            ...
            if (i < 61 && !sjObj.opt.Year && !sjObj.opt.Month
              && !sjObj.opt.Day && !sjObj.opt.Hour) {
              this.mArr[i] = i;
          }
        }
      },
...
//          console.log(this);
if (df.width) {
  str = '<div class="df-class">' + text
  + '</div><div class="df-item " style="height:'
  + (df.height * 5 - 1) + 'px;min-width:'
  + df.width
  + 'px"><ul class="df-ul" data-class=' + c + '>';
} else {
  str = '<div class="df-class">' + text
  + '</div><div class="df-item " style="height:'
  + (df.height * 5 - 1)
...
return str;
        ...
        strEnd : function() {
          var df = this;
          return "</ul><div class='G-bg'><div class='G-top' style='height:"
          + (df.height * 2)
          + "px'></div><div class='G-mid' style='height:"
          + df.height
          + "px'></div><div class='G-btm' style='height:"
          + (df.height * 2) + "px'></div></div></div>"
        },
        //数字小于10 就在字前面+一个0
        fillZero : function(x) {
          if (x < 10) {
...
var endYear = this.endYear || parseInt(sjObj.opt.y)
...
for (var x = y, i = 0; y > 0 ? x != 0 : x < 1; y > 0
  ? x--
  : x++, i++) {
  if (y < 0) {
    sjObj.opt.yyArr[i] = nowTime.getFullYear() + x;
...
sjObj.opt.getArr()
        ...
        setCenter : function() {
          ...
          if (tabWid > wid) {
            $(".df-wrap table").each(function() {
              mWid = parseFloat($(this).width()) > mWid
        ...
        buildArrStr : function(Arr, txt, c) {
          ...
          $.each(Arr, function() {
            str += '<li class="df-li df-show"  data-val='
            + sjObj.opt.fillZero(this)
            + ' style="line-height:'
            + sjObj.opt.height + 'px;height:'
            + sjObj.opt.height + 'px">'
            + sjObj.opt.fillZero(this) + '</li>'
          })
//          console.log(Arr)
...
sjObj.opt.timeElm.append(sjObj.opt.t_box.append(main
  .append(wrap)));
            //  }
          } else {
            sjObj.opt.timeElm = $("<div class='df-persp'><div class='persp-bg'></div>");
            sjObj.opt.timeElm.append(sjObj.opt.t_box
              .append(sjObj.opt.finalshow
                ? sjObj.opt.df_final
                : "").append(main.append(wrap))
              .append(sjObj.opt.df_btn));
          ...
          if (sjObj.opt.ampmText) {
            main
            .append("<div class='df-wrap'><table><tbody><tr><td>"
              + sjObj.opt.buildAmPmStr()
              + "</tr></tbody></table></div>")
          }
          if (sjObj.opt.Format == "dd-mm-yyyy") {
            if (sjObj.opt.Day)
              $(sjObj.opt.timeElm.find('.df-wrap')[0]).find('tr')
            .append("<td>"
              + sjObj.opt.buildArrStr(
                sjObj.opt.ddArr,
                sjObj.opt.dayText, "dd")
              + "</td>");
            if (sjObj.opt.Month)
              $(sjObj.opt.timeElm.find('.df-wrap')[0]).find('tr')
            .append("<td>"
          ...
          if (sjObj.opt.Hour) {
            ...
            $(eml).find('tr').append("<td>"
              + sjObj.opt.buildArrStr(sjObj.opt.hArr,
                sjObj.opt.hourText, "h") + "</td>")
            main.append(eml);
          };
          if (sjObj.opt.Minute) {
            var eml = sjObj.opt.df_wrap();
            $(eml).find('tr').append("<td>"
              + sjObj.opt.buildArrStr(sjObj.opt.mArr,
                sjObj.opt.minuteText, "m") + "</td>");
            main.append(eml);
          };
          if (sjObj.opt.Seconds) {
            var eml = sjObj.opt.df_wrap();
            $(eml).find('tr').append("<td>"
              + sjObj.opt.buildArrStr(sjObj.opt.sArr,
                sjObj.opt.secondsText, "m") + "</td>");
            main.append(eml);
          }
...
if (valarr.length == 2) {
  ...
  && (valarr.indexOf("-") != -1 || valarr
    .indexOf("/") != -1)) {
  str += getnyrstr(valarr[0]) + " "
  + getsfmstr("")
//                console.log("组合 2");
} else {
...
if (sjObj.opt.Year)
  sjObj.opt.timeElm
.find("[data-class='yyyy'] .df-li").each(
  function() {
    console
    .log(
      parseInt($(this)
...
if (sjObj.opt.Month)
  sjObj.opt.timeElm.find("[data-class='mm'] .df-li")
.each(function() {
...
if (sjObj.opt.Day)
  sjObj.opt.timeElm.find("[data-class='dd'] .df-li")
.each(function() {
...
if (sjObj.opt.Hour)
  sjObj.opt.timeElm.find("[data-class='h'] .df-li")
.each(function() {
...
if (sjObj.opt.Minute)
  sjObj.opt.timeElm.find("[data-class='m'] .df-li")
.each(function() {
...
.find("[data-class='yyyy'] .df-li").each(
  function() {
    if (parseInt($(this)
      .attr("data-val")) == parseInt(sjObj.opt.yyyy)) {
      var pY = -($(this).index() - 2)
    * sjObj.opt.height;
    $(this).parent().css({
      "transform" : "translate(0,"
...
sjObj.opt.bindFun();
        ...
        bindFun : function() {
          ...
          sjObj.opt.timeElm.find(".df-ok").on("click", function() {
            ...
            // 判断月为true,日为false,显示去掉-dd
            if (sjObj.opt.Month && !sjObj.opt.Day) {
              str = str.replace("-dd", '')
            }
        ...
        getFinal : function() {
          ...
          if (sjObj.opt.showNowTime) {
            sjObj.opt.timeElm.find(".df-ul").each(function() {
              ...
              var val = $($(this).find(".df-li")[Math
                .round(currentY / sjObj.opt.height)
                + 2]).attr("data-val");
              sjObj.opt.vardata(dataClass, val);
              // console.log(dataClass, val)
              $(this).unbind("webkitTransitionEnd").on(
              ...
              daysJudge : function(name) {
                if (name == 'mm' || name == "yyyy") {//在选择年份获取天数时
                  var day = new Date(sjObj.opt.yyyy, sjObj.opt.mm, 0)
                  .getDate();
                  var l = sjObj.opt.timeElm.find('[data-class="dd"]')
                  .find(".df-show").length
                  ...
                  if (mubiao > 0) {
                    ...
                    for (var i = 0; i < mubiao; i++) {
                      $(sjObj.opt.timeElm.find('[data-class="dd"]')
                      .find(".df-li")[l + i])
                      .removeClass("df-hide")
                      .addClass("df-show")
                    ...
                    //console.log("小于");
                    var naomovey = getTranslateY(sjObj.opt.timeElm
                    .find('[data-class="dd"]'))
                    for (var i = 0; i > mubiao; i--) {
              ...
              moveElm : function(eml) {
                ...
                return $(eml).each(function() {
                  //移动事件变量
                  //console.log("进入",$(this))
                  var sX = null, sY = null, mX = null, mY = null, eX = null, eY = null, sTime = null, eTime = null, mTime = null, nTime = null, //开始时间，结束时间，移动时的时间，开始到现在花费的时间
                  nY = 0, drt = null, nowElm = null, //现在的Y位置。方向，当前元素
                  canStart = true, canMove = false, canEnd = false, //移动事件条件。。
                  emlLang = null, //子元素长度
                  maxY = null, minY = null, //最大距离和最小距离
                  lastY = null, nowY = null, moveY = null, stopInertiaMove = false, //是否停止惯性滚
                        ...
                        function inertiaMove() {
                          ...
                          if (dir * nowV > 0) {//大于0是减速停止
                            ...
                            if (moveCy > sjObj.opt.maxY) {
                              ...
                              var MC = Math.round(moveCy
                              / sjObj.opt.height)
                              //    console.log(MC)
                              if (MC > 2) {
                                //      console.log("大于长度")
                                MC = 2
                              } else if (MC < -(emlLang - 1)
                              + 2) {
                ...
                $(this).unbind("touchstart mousedown").on(
                "touchstart mousedown", moveStart)//触摸起始//鼠标按下
